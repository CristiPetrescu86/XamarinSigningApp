...
namespace SigningApp.XadesSignedXML.XML
{
	...
	public class UnsignedSignatureProperties
	{
			...
			try 
			{
				while (enumerator.MoveNext()) 
				{
					...
					if (iterationXmlElement != null)
					{
						...
						{
							newXadesSignedXml = new SignedXml(counterSignedXmlElement);
						}
						else
						{
							newXadesSignedXml = new SignedXml();
						}
		...
		public XmlElement GetXml()
		{
            ...
            retVal = creationXmlDocument.CreateElement("xades", "UnsignedSignatureProperties", SignedXml.XadesNamespaceUri);

			if (this.counterSignatureCollection.Count > 0)
			{
				foreach (SignedXml xadesSignedXml in this.counterSignatureCollection)
				{
					bufferXmlElement = creationXmlDocument.CreateElement("CounterSignature", SignedXml.XadesNamespaceUri);
