...
namespace SignatureXML.Library
{
	...
	public class OCSPIdentifier
	{
		...
		public XmlElement GetXml()
		{
            ...
            retVal = creationXmlDocument.CreateElement("xades", "OCSPIdentifier", SignedXml.XadesNamespaceUri);

			retVal.SetAttribute("URI", this.uriAttribute);

			if (!String.IsNullOrEmpty(this.responderID))
			{
                bufferXmlElement = creationXmlDocument.CreateElement("xades", "ResponderID", SignedXml.XadesNamespaceUri);
			...
			if (this.producedAt != DateTime.MinValue)
			{
                bufferXmlElement = creationXmlDocument.CreateElement("xades", "ProducedAt", SignedXml.XadesNamespaceUri);
